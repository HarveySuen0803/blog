...
- eg: db_user 拆分成 db_user_1, db_user_2, db_user3 三个库, 通过 id % 3 的方式选择不同的库进行访问

水平分表, 将一个表的数据拆分到多个表中, 通过 Id 取模的方式路由到不同的表中

- eg: tb_user 拆分成 tb_user_1, tb_user_2, tb_user3 三个表, 通过 id % 3 的方式选择不同的表进行访问
...
MySQL 8 可以让 UUID 的时间序列的高位放在前面, 可以保证单调递增, 同时还去除了无意义的字符, 并且通过 Binary 存储, 将一个 UUID 的占用从 36B 降低到了 16B, 非常强悍, 用来做 Primary Key 太适合了.
