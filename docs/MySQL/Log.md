...
Two Phase Commit (2PC) 是一种原子提交协议, 用于协调参与分布式原子事务的所有进程, 决定提交或回滚, , 该协议在许多临时系统失败的情况下依然能实现其目标, 因此得到了广泛的使用. 然而, 两阶段提交协议并不能抵御所有可能的失败配置, 在极少数情况下, 需要人工干预来纠正结果, 为了从失败中恢复 (大部分情况下是自动的), 协议的参与者使用日志记录协议的状态.

Two Phase Commit 由两个阶段组成, 如果这两个阶段的数据不一致, 则会进行回滚, 保证数据一致性.

- Prepare Phase: 执行一条更新语句时, 首先会将相关的数据写入内存缓冲区, 再写入 Redo Log, 标记此时状态为 Prepare, 表示此事务已经准备好提交, 但仍可能失败或回滚.
- Commit Phase: 写入数据到 Bin Log, 修改状态为 Commit, 表示数据的更改已经被永久保存在数据库中.
